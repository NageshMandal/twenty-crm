...
const Content: React.FC<Props> = ({
  ...
  const getMessage = async () => {
    try {
      if (currentThread) {
        const res = await inboxApi.getAiSdrMessages(currentThread?.id, workflowId);
        setMessages(res?.data);
        setMessage((res as any)?.preparedReply || "");
      }
    } catch (error) {
                    ...
                    <div className='flex justify-end w-full'>
                      <div className='flex items-start gap-10 max-w-800'>
                        <div className='flex flex-col gap-4'>
                          ...
                          <p className='px-4 title-2 text-12'>Event at: {item.event_at}</p>
                        </div>
                        <ReactAvatar
