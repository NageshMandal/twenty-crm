...
const customFormatOptionLabel = (option: any) => (
  <div>
    ...
    <div>Subject: {option.subject}</div>
    <div>Created: {option.created_at}</div>
    <div>Modified: {option.updated_at}</div>
  </div>
);

const ReactSelectCard: React.FC<Props> = ({
  label = "",
  isAuth,
  options,
  isMulti = false,
              ...
              option: (state) =>
                classNames(
                  "!text-13 !leading-18 !text-neutral-800 dark:!text-neutral-300 !px-4 !cursor-pointer ",
                  {
                    ...
                    ["!bg-transparent dark:!bg-transparent"]: !state.isFocused,
                  }
                ),
              menu: () =>
...
export default ReactSelectCard;
